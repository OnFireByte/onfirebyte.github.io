...
ในโลกของ FP มันจะมี concept ของ **Wrapper** ที่เป็น type ที่มันครอบ value type นึงอีกที

ถ้าพูดแบบเห็นภาพก็คือมันเป็นกล่องที่เอาไว้ในของ**ประเภทนึง** ซึ่งเราสามารถเขียน Wrapper เป็น type ได้เป็นหน้าตาแบบนี้
...
จะเห็นได้ว่า Wrapper มันมีนิยามที่กว้างมาก ๆ ทั้งๆ ที่ array กับ promise เป็น wrapper เหมือนกัน แต่หน้าที่และความสามารถมันต่างกันโดยสิ้นเชิง ซึ่งไอความรู้สึกแบบนี้มันก็จะเกิดขึ้นเหมือนกันกับตัว Monad เอง

โดยหัวใจหลักของ Monad เหนี่ยมันพูดถึงกลุ่มของ Wrapper จำนวนนึง ที่**มี method/function/operation 2 ตัว ซึ่งก็คือ unit กับ bind** ถ้า Wrapper ไหนมี function นี้ก็ถือว่าเป็น Monad แล้ว
...
ปล. ส่วนใหญ่มักจะเรียกว่า return แต่ป้องกันการสับสนกับ keyword return ใน JS/TS ก็เลยเรียก unit แทน
...
จริง ๆ แล้ว ที่ว่ามาทั้งหมดเหนี่ย หัวใจหลักของ Monad มันก็คือ function bind นี่แหละ ดังนั้นแนะนำว่าให้ทำความเข้าใจส่วนนี้เยอะ ๆ
...
หรือมองอีกมุมนึง เรามองว่า ปกติแล้วถ้า f เป็น `(t:T) => M<U>)` ถ้าเราใช้ map มันก็จะได้ออกมาเป็น `M<M<U>>` แต่ขั้นสุดท้ายเราทำมันให้เหลือชั้นเดียว เป็น `M<U>` ซึ่งมันก็คือการ flatten Wrapper เรา ทำให้หลาย ๆ ครั้ง function bind มักจะถูกเรียกว่า **_flatMap_** (เริ่มคุ้น ๆ แล้วใช่ปะ)
...
อย่างที่แอบสปอยล์ไปข้างต้นว่า bind มีอีกชื่ือนึงก็คือ flatMap ซึ่งมันก็คือ flatMap เดียวกันกับใน array ของ JS นั่นแหละ ดังนั้นก็หมายความว่า array ก็เป็น monad ตัวนึง (น่าจะเป็นตัวเดียว) ใน JS
...
`Maybe<T>` (หรือบางทีก็เรียก `Option<T>`) เป็นแนวคิดนึงที่ programmer ส่วนใหญ่อาจจะไม่เคยได้ยิน แต่จริง ๆ แล้วมันก็คือการมองค่าที่สามารถเป็น null ได้ (nullable) มาเป็น Wrapper ตัวนึง

หลาย ๆ คนน่าจะเคยประสบพบเจอกับ bug ที่เกิดจากการที่ value ที่ได้มามันดันเป็น null โดยไม่รู้ตัวใช่ไหม? ไอปัญหานี้จริง ๆ แล้วมันใหญ่มาก ๆ จนคนสร้างภาษา ALGOL W ที่เป็นต้นน้ำของการใช้ null ถึงกับบอกเองว่า null เป็น **_The Billion Dollar Mistake_** เพราะไอบัค null reference นี่มันทำให้บริษัทหลาย ๆ ที่เสียหายรวมกันเป็นพันล้านแล้ว (ซึ่งตอนนี้ก็น่าจะเกินแล้ว 5555)

โอเค ในเมื่อ null มันไม่ดี แต่เราก็อยากที่ของสักอย่างที่เป็นตัวแทนของความไม่มี (nothingness) ทำให้มันมีหลายไอเดียมาก ๆ ซึ่งไอเดียนึงที่ใช้กันเยอะในภาษา FP ก็คือ Maybe

ถ้าให้พูดง่าย ๆ Maybe มันเป็น Wrapper ที่เก็บค่าพร้อมกับ state เอาไว้ โดย state จะเป็นตัวบอกว่าจริง ๆ แล้วมันมีค่าจริง ๆ หรือ เป็น Nothing กันแน่
...
ใน code ตัวนี้อาจจะไม่เห็นชัดเจนว่ามันดีกว่าการใช้ null ยังไง อันนี้แนะนำว่าลองไปอ่าน `Option<T>` ของ rust ดู ซึ่งหัวใจหลักก็คือในภาษาที่ใช้ maybe มันจะ mechanism ที่บังคับให้เราต้องเช็คตลอดว่าของใน Maybe มันมีอยู่จริง ๆ ไหม

ต่อมา จะเห็นได้ว่า maybe มันจะมี bind ซึ่งหลักการมันคือ **_ถ้าตัวมันเป็น null มันจะไม่รัน function f เลย แล้วคืน empty มาแทน_** ซึ่ง mechanism แบบนี้ทำให้เราสามารถ ใช้เรียก function เป็นทอด ๆ ยาว ๆ ได้โดยไม่ต้องมานั่งเช็คระหว่างทางว่าเป็น null ไหม
